<?php

if(!function_exists('_log')){
  function _log( $message ) {
    if( WP_DEBUG === true ){
      if( is_array( $message ) || is_object( $message ) ){
        error_log( print_r( $message, true ) );
      } else {
        error_log( $message );
      }
    }
  }
}


class Comm100LiveChat
{
	// singleton pattern
	protected static $instance;
	public static $service_url = 'http://hosted.comm100.com/AdminPluginService/livechatplugin.ashx';
	//public static $service_url = 'http://192.168.8.48/plugin/livechatplugin.ashx';

	/**
	 * Absolute path to plugin files
	 */
	protected $plugin_url = null;
	protected $site_id = null;
	protected $plan_id = null;

	/**
	 * Starts the plugin
	 */
	protected function __construct()
	{
		add_action('widgets_init', create_function('', 'register_widget("Comm100LiveChatWidget");'));
	}

	public static function get_instance()
	{
		if (!isset(self::$instance))
		{
			$c = __CLASS__;
			self::$instance = new $c;
		}

		return self::$instance;
	}

	/** 
	 * Returns plugin files absolute path
	 *
	 * @return string
	 */
	public function get_plugin_url()
	{
		if (is_null($this->plugin_url))
		{
			$this->plugin_url = WP_PLUGIN_URL.'/wp-comm100-livechat/plugin_files';
		}

		return $this->plugin_url;
	}

	public function is_installed()
	{
		return $this->get_site_id() > 0;
	}

	public function get_site_id()
	{
		if (is_null($this->site_id))
		{
			$this->site_id = get_option('comm100livechat_site_id');
		}

		// siteId must be >= 0
		// also, this prevents from NaN values
		$this->site_id = max(0, $this->site_id);

		return $this->site_id;
	}	
	public function get_code($site_id, $plan_id) {
		$result = $this->request(sprintf('?action=code&siteId=%s&planId=%s', $site_id,  $plan_id));
		if (!is_wp_error($result) && $result['success']) {
			return $result['response'];
		}
		else {
			return '';
		}
	}

	public function request($url, $session = '', $headers = array()) {
		$http = new WP_Http;
		$headers = array('Cookie'=>'ASP.NET_SessionId='.$session);
		$url = Comm100LiveChat::$service_url.$url;
		
		_log($url);

		$result = $http->request($url, array('timeout' => 60, 'method'=>'get', 'headers'=>$headers));


		if (is_wp_error($result)) {
			_log($result);
			return $result;			
		}
		else {
			return json_decode($result['body'], true);
		}
	}
}

class Comm100LiveChatWidget extends WP_Widget
{
	public function __construct() {
		parent::__construct('comm100livechat_widget', 'Comm100 Live Chat', 
			array('description' => 'Install Chat Button to let your visitors start a chat with you.'));
	}

	public function update( $new_instance, $old_instance ) {
		$instance = $old_instance;

		$instance['plan_id'] = strip_tags($new_instance['plan_id']);

		$site_id = Comm100LiveChat::get_instance()->get_site_id();

		$instance['code'] = Comm100LiveChat::get_instance()->get_code($site_id, $instance['plan_id']);
		
		return $instance;
	}

	public function form($instance)
	{
		$plan_id = isset($instance['plan_id']) ? $instance['plan_id'] : '0';
		$site_id = Comm100LiveChat::get_instance()->get_site_id();

		$result = Comm100LiveChat::get_instance()->request('?action=plans&siteId='.$site_id, '');
		if (!is_wp_error($result) && $result['success']) {
			$plans = $result['response'];
		}
		else {
			$plans = array();
		}

		if (count($plans) > 1) {
			if ($plan_id == '0') {
				$plan_id = $plans[0]['id'];
			}
?>
			<div>
				Select which code plan to use on my site: 
				<select id="<?php echo $this->get_field_id('plan_id'); ?>" 
					name="<?php echo $this->get_field_name('plan_id'); ?>" 
					value="<?php echo $plan_id ?>" style="width:110px">
					<?php for ($i=0;$i<count($plans);$i++) { ?>
						<option value="<?php echo $plans[$i]["id"] ?>" <?php if ($plans[$i]["id"] == $plan_id) echo 'selected=selected'?> ><?php echo $plans[$i]["name"] ?></option>
					<?php } ?>
				</select>
			</div>
			<div style="margin-top:5px;">
				<script type="text/javascript">
					function open_customize(plan_field_id) {
						window.open('http://hosted.comm100.com/LiveChatFunc/PlanDetailManage.aspx?siteId=<?php echo $site_id; ?>&codeplanId='
							+ document.getElementById(plan_field_id).value + '&ifEditPlan=true');
					}
				</script>
				Click <a href="#" onclick="open_customize('<?php echo $this->get_field_id('plan_id'); ?>');return false;" target="_blank">here</a> to customize this code plan.
			</div>
<?php 	} 
		else {
			$instance['plan_id'] = strip_tags($plans[0]['id']);
?>
			<div><a target="_blank" href="http://hosted.comm100.com/LiveChatFunc/PlanDetailManage.aspx?siteId=<?php echo $site_id; ?>&codeplanId=<?php echo $plan_id?>&ifEditPlan=true">Customize</a> my live chat to better match the look and feel of my website.</div>
			<input type="hidden" id="<?php echo $this->get_field_id('plan_id'); ?>" 
				value="<?php echo $instance['plan_id']; ?>" name="<?php echo $this->get_field_name('plan_id'); ?>"/>
<?php   }

	}

	public function widget($args, $instance)
	{
		if (Comm100LiveChat::get_instance()->is_installed()) {
			echo $instance['code'];	
		}		
	}
}



